...
**Amabilidad.** Caída continua de 25,7 s en `cleanupCatch.js`, sin rondas: caen 32 objetos —22 latas y 10 plantas, siempre los mismos— y se recogen las latas con un cesto que sigue al dedo. Una planta en el cesto descuenta una lata. La cantidad la fija la config y el azar solo elige **el orden y la columna**, así que el denominador del marcador 0–1000 es constante y la mejor marca es comparable consigo misma. La curva es la velocidad de caída: 2.650 ms en cruzar la pista el primer objeto y 1.450 ms el último (−45 %); `fallVariation` rota tres ritmos por índice para que en pantalla no caigan dos iguales seguidos sin mover la dificultad total. El cesto **persigue** al dedo a velocidad tope (181 ms de punta a punta) en vez de teletransportarse, que si no un arrastre barría todo lo que hubiera en medio. Se juega también con las flechas del teclado.

Dos propiedades medidas con `tools/design/kindness-reach.cjs`, que son las que hacen que el +5 no dependa del sorteo: **ningún par de objetos coincide en la banda del cesto** (80–90 % de su caída), así que nunca hay que elegir entre una lata y una planta; y sobre 10.000 partidas con columnas reales **no hay una sola transición imposible**, con 34 ms de margen en la más ajustada. El borde inferior de la banda está en el 90 % y no en el 94 % justamente por eso: una lata desaparece al entrar en la banda, pero una planta hay que seguir esquivándola hasta que sale, y con .94 quedaban solo 71 ms para cruzar la pista después.

**Cuánta prisa hay es lo que se afina, no la velocidad.** Con 18+8 el minijuego salía fácil, y la
medida decía por qué: solo el **2 %** de los pasos de un objeto al siguiente dejaba menos de 250 ms
de margen, o sea que casi nunca había que correr. Con 22+10 ese porcentaje sube al **17 %** y el peor
caso baja de 167 ms a 34. Por encima no se puede subir: a **34 objetos** el margen se vuelve negativo
y el +5 empieza a depender de dónde caigan las columnas, que es justo lo que este diseño evita.
...
Fuerza dispone de cinco rondas de 3,0 a 4,2 s —la ventana crece a medida que la zona se estrecha, de modo que el margen extra llega como ~2,5 pasadas por la zona en la primera ronda y ~3,5 en la última— y puntúa precisión con máxima puntuación en el núcleo de una zona que cambia de sitio cada ronda. Amabilidad no tiene rondas: 25,7 s de caída continua con 32 objetos, y su dificultad es la velocidad de caída (2.650 → 1.450 ms en cruzar la pista) junto a lo seguidos que llegan. Estilo ofrece cinco trazados progresivos, sin tiempo máximo ni tempo obligatorio. La preparación de 1 s queda fuera de estos plazos; cada ventana comienza cuando se presenta su contenido.
