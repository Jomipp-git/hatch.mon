...
**Motivo por tipo.** `Type1` elige un patrón geométrico de la biblioteca de `shellSkins.js` (`waves`, `bolts`, `sparks`, `sprouts`, `orbits`, `diamonds`, `dots`, `bands`, `crystals`, `scales`, `blocks`, `eclipse`), que cubre el repertorio entero sin trabajo manual; `assets/skins/motifOverrides.json` pisa lo que convenga mimar a mano, respetando la regla de que la forma base va lisa. Cada motivo es una baldosa SVG de 48 px al 17 % de opacidad, repetida sobre el plástico: textura, no dibujo encima. El patrón se compone en `--motifImage`/`--motifSize` y un motivo desconocido deja la carcasa lisa en lugar de romperla. La carcasa pertenece a la especie que has tenido, no a la que has mantenido viva mucho tiempo: se desbloquea al eclosionar y de nuevo en cada evolución, sin esperar a MADURO. Además se rellena desde la Pokédex con toda forma que registre `ownedIds`, que solo se anota al convertirse en tu compañero, así que quien crió una especie antes de este cambio conserva su carcasa. Se comprueba en la eclosión, en la evolución, durante los pasos de simulación (incluido tiempo offline) y al renderizar. Cada forma se registra una sola vez.
