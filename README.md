...
**Reglas antes de jugar.** La primera práctica de cada actividad abre una ficha con los tres pasos y cuántas rondas tiene, con *Empezar* y *Ahora no*. Va **antes** de `begin`, así que leerlas y echarse atrás no cuesta una sesión. El interrogante de la tarjeta las reabre cuando se quiera, y tampoco cobra. Que ya se han visto se guarda en `hatch.mon.tutorials`, fuera de la partida: no tiene sentido volver a explicar el minijuego al eclosionar otro huevo, y el save no cambia de esquema por esto.
...
**Paleta por línea evolutiva.** Una carcasa tiene que decir de quién es antes de leer el nombre, y antes las 75 eran la misma en distinto tono. Ahora el número de colores crece con la distancia a la forma base de la línea (contada por `PreEvolutionId`): una forma base va **monocroma y sin motivo**, y cada evolución suma un color tomado del propio sprite. El muestreo exige tonos realmente distintos —no otra sombra del mismo— y descarta el contorno negro y el blanco del brillo, que como motivo leerían a mancha; si el sprite no da para más, se emiten menos colores en vez de inventar uno.

**Motivo.** Manda la silueta de la especie si está dibujada a mano en `shellSkins.js` (`charizard`, `jynx`, `electabuzz`, `snorlax`, `togekiss`, `toxtricity`); si no, la familia geométrica que toca por `Type1` (`waves`, `bolts`, `sparks`, `sprouts`, `orbits`, `diamonds`, `dots`, `bands`, `crystals`, `scales`, `blocks`, `eclipse`), que cubre el repertorio entero sin trabajo manual. Las siluetas no dibujan al Pokémon entero sino su elemento reconocible —la llama de Charizard, la púa de Toxtricity, la «Z» de Snorlax—, que es como funciona un estampado y lo único legible a 24 px. `assets/skins/motifOverrides.json` sigue siendo el gancho para pisar cualquier tema a mano. Una silueta declarada en el generador y no dibujada en `shellSkins.js` **para el build**.
...
**Dos superficies.** El mismo dibujo cambia de papel según dónde cae: en el fondo de página va a 104 px y 42 % de opacidad, que es donde hay lienzo para que se lea; en la carcasa, a 58 px y 16 %, porque son 16 px de marco alrededor de la pantalla y no admiten más. Se componen en `--motifImagePage`/`--motifSizePage` y `--motifImage`/`--motifSize`, y el fondo de página se tiñe con `--pageBase`. Un motivo desconocido deja ambas superficies lisas en lugar de romperlas.
