...
export const CACHE_KEYS=['hatch.mon.v3','hatch.mon.shells','hatch.mon.displayMode','hatch.mon.language'];
const REVISION_KEY='cloud-revision',UPDATED_AT_KEY='cloud-updated-at';
const systemText=(key,vars)=>globalThis.HatchI18n?.t(key,vars)??key;
...
export function snapshotCache(cache){
 ...
 game.message='';
 return {game,preferences:{shells:JSON.parse(cache.getItem(CACHE_KEYS[1])||'null'),displayMode:cache.getItem(CACHE_KEYS[2])||'color',language:cache.getItem(CACHE_KEYS[3])==='en'?'en':'es'}};
}
export function hydrateCache(cache,envelope){
 ...
 cache.setItem(CACHE_KEYS[3],envelope.preferences?.language==='en'?'en':'es');
}
...
export function createCloudSaveService({client,session,cache,notify=()=>{},delay=15000,onRemote=()=>{}}){
 ...
 async function flush({drain=true}={}){
  ...
  }catch{pending=pending||payload;cache.setItem('cloud-dirty','1');notify(systemText('system.unsynced'));return false;}}
  if(!pending)notify(cache.getItem('cloud-backup')?systemText('system.pendingBackup'):'');return true;})();
 ...
 function queue({immediate=false}={}){if(closed)return;try{const next=snapshotCache(cache);if(!next)return;if(!pending&&!inFlight&&canonicalSnapshot(next)===base())return;cache.setItem('cloud-dirty','1');if(blocked)return;pending=next;}catch{notify(systemText('system.savePreparationFailed'));return;}if(immediate){urgent=true;clearTimeout(timer);timer=null;queueMicrotask(()=>void flush({drain:false}));}else if(!timer&&!inFlight)timer=setTimeout(()=>void flush({drain:false}),delay);}
