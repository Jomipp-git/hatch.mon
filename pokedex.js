...
globalThis.Pokedex=(()=>{
 const fresh=()=>({});
 function record(dex,id,event='seen',individualId=null){
  const key=PokemonData.canonicalId(id);if(!key)return;
  const entry=dex[key]??={seen:false,ownedIds:[],evolved:false,bred:false,received:false};entry.seen=true;
 ...
 return Object.freeze({fresh,record,weights,choose,valid});
})();
