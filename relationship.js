...
globalThis.Relationship=(()=>{
 ...
 function reward(s,event){if(!s.relationship||!['alive','critical'].includes(s.phase))return;const gain=RELATIONSHIP_CONFIG.rewards[event]||0;s.relationship.points=Math.min(RELATIONSHIP_CONFIG.max,s.relationship.points+gain);}
 ...
 function interact(s){
  ...
  if(s.lightsOff){s.lightsOff=false;penalty();return {kind:'wake',emotion:'Surprised',message:'Se ha despertado sobresaltado. Necesitaba descansar.'};}
  if(s.pokerus||s.phase==='critical')return {kind:'sick',emotion:'Worried',message:'No se encuentra bien. Acompáñalo con calma.'};
  if(s.care.energia<20)return {kind:'tired',emotion:'Sad',message:'Está cansado. Le vendrá bien descansar.'};
  if(r.attention>RELATIONSHIP_CONFIG.positiveTapLimit){penalty();return {kind:'startled',emotion:'Angry',message:'Necesita un poco de espacio.'};}
  reward(s,'touch');s.care.felicidad=Math.min(100,s.care.felicidad+RELATIONSHIP_CONFIG.positiveMood);return {kind:'happy',emotion:r.points>=RELATIONSHIP_CONFIG.closeBond?'Joyous':'Happy',message:r.points>=RELATIONSHIP_CONFIG.closeBond?'Reconoce tu cariño y se acerca.':'Le gusta que estés aquí.'};
 }
