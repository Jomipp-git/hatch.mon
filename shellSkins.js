...
globalThis.ShellSkins=(()=>{
 ...
 try{const raw=JSON.parse(globalThis.localStorage?.getItem(key)||'null');if(raw&&Array.isArray(raw.unlocked)){saved.unlocked=[...new Set(raw.unlocked.filter(id=>Object.hasOwn(SHELL_THEMES,id)))];if(raw.selected==='default'||saved.unlocked.includes(raw.selected))saved.selected=raw.selected;}}catch{}
 const persist=()=>{try{globalThis.localStorage?.setItem(key,JSON.stringify(saved));}catch{}};
