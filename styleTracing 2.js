...
globalThis.StyleTracing=(()=>{
 ...
 function mount({field,hint,status,node,onFinish}){
  ...
  let round=0,scorer=createScorer(0),pointer=null,disposed=false,centered=true,trail=[],scores=[];
  function draw(){
  ...
  const position=e=>{const r=canvas.getBoundingClientRect();return [(e.clientX-r.left)*config.width/r.width,(e.clientY-r.top)*config.height/r.height];};
  canvas.addEventListener('pointerdown',e=>{if(disposed||pointer!==null||(e.button!==undefined&&e.button!==0))return;e.preventDefault();if(!scorer.start(position(e)))return;pointer=e.pointerId;canvas.setPointerCapture(e.pointerId);trail=[position(e)];draw();});
  canvas.addEventListener('pointermove',e=>{if(disposed||e.pointerId!==pointer)return;e.preventDefault();for(const sample of e.getCoalescedEvents?.().length?e.getCoalescedEvents():[e]){const p=position(sample);centered=scorer.move(p);trail.push(p);if(trail.length>12)trail.shift();}canvas.dataset.centered=String(centered);draw();});
