...
born();snapshot();assert.equal(run('forceEvolution("magmar")'),true);preserved();assert.equal(run('state.age/HOUR'),24);assert.equal(run('state.training.style'),50);for(const k of ['iq','strength','kindness'])assert.equal(run(`state.training.${k}===before.training.${k}`),true);assert.ok(run('state.relationship.points>=PokemonData.rules("magby")[0].MinBond'));assert.equal(run('Vital.getLifeStage(state)'),'ADULTO');
...
// La edad se toma de la propia regla: la prueba es del consumo del objeto, no del valor de balance.
born('pikachu');run('state.age=current().rules[0].ageMs');assert.equal(run('evolve(current().rules[0],"thunder")'),false);run('state.inventory.thunder=1');assert.equal(run('evolve(current().rules[0],"thunder")'),true);assert.equal(run('state.inventory.thunder'),0);
