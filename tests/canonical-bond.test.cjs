...
assert.equal(run('bondRules.length'),26);assert.equal(run('bondRules.filter(r=>r.MinBond===1).length'),1);assert.equal(run('bondRules.filter(r=>r.MinBond===2).length'),16);assert.equal(run('bondRules.filter(r=>r.MinBond===3).length'),7);assert.equal(run('bondRules.filter(r=>r.MinBond===4).length'),1);assert.equal(run('bondRules.filter(r=>r.MinBond===5).length'),1);
...
for(let i=0;i<run('bondRules.length');i++){
 ...
 // El libro trajo reglas que además piden una acción repetida (Bayleef→Meganium, 30 limpiezas) o
 // un objeto (Kirlia→Gallade, Piedra Alba). Se satisfacen aquí para que lo único que falte sea el
 // Vínculo, que es lo que esta prueba mide.
 run('state.pokemonId=from;state.age=r.ageMs;state.training={iq:0,strength:0,kindness:0,style:0,...r.training};state.care.felicidad=100;state.sustained={};for(const c of r.sustained){state.care[c.estadistica]=100;state.sustained[sustainedKey(c)]=c.duracionMs;}state.actions={};if(r.action)state.actions[r.action]=r.repetitions;if(r.item)state.inventory[r.item]=1;state.relationship.points=0;var minimum=source.MinBond*RELATIONSHIP_CONFIG.perHeart;');
 ...
 {
  // `evolve` exige que el objeto que se le pasa sea exactamente el que pide la regla, asi que con
  // las reglas de objeto (Kirlia→Gallade) hay que entregarselo.
  run("state.relationship.points=minimum-.01");assert.equal(run('evolve(r,r.item||null)'),false);
  run('state.relationship.points=minimum');assert.equal(run('evolve(r,r.item||null)'),true);
...
for(const blank of ['null','undefined','""']){
 // Sin MinBond la regla tiene que cumplirse sola, pero el resto de sus requisitos siguen ahi: el
 // estado se prepara desde la propia regla en vez de heredar el de la ultima vuelta del bucle.
 run(`var absent=canonicalRule({...bondRules[0],MinBond:${blank},MinAgeDays:0});state.relationship.points=0;
  state.pokemonId=PokemonData.legacyId(bondRules[0].FromId);state.age=absent.ageMs;
  state.training={iq:0,strength:0,kindness:0,style:0,...absent.training};state.sustained={};
  for(const c of absent.sustained){state.care[c.estadistica]=100;state.sustained[sustainedKey(c)]=c.duracionMs;}
  state.actions={};if(absent.action)state.actions[absent.action]=absent.repetitions;`);
 assert.equal(run('absent.minBond'),null);assert.equal(run('conditionsMet(absent)'),true);
}
for(const value of ['-1','6','NaN','"2"'])assert.throws(()=>run(`canonicalRule({...bondRules[0],MinBond:${value}})`));
console.log('PASS 26 canonical Bond rules, all playable: 40/60 points, below/exact/above, high Mood insufficient, Oak=engine, all age/training gates, evolution/Force paths; nulls and invalid units.');
