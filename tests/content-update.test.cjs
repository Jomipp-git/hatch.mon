...
assert.equal(loaded.run('state.inventory.berryStrength'),undefined);assert.equal(loaded.run('state.coins'),150);assert.equal(loaded.run('state.pokedex[PokemonData.canonicalId("pichu")].evolved'),true);assert.equal(loaded.run('displayMode'),'lcd');assert.equal(loaded.run('validSave(state)'),true);}
 const SHOP_SLOTS=JSON.parse(run('JSON.stringify(SHOP_STAPLES)')).length+3;
 const today=run('JSON.stringify(dailyShop({getFullYear:()=>2026,getMonth:()=>8,getDate:()=>10}))'),tomorrow=run('JSON.stringify(dailyShop({getFullYear:()=>2026,getMonth:()=>8,getDate:()=>11}))');assert.equal(new Set(JSON.parse(today)).size,SHOP_SLOTS);assert.equal(today,run('JSON.stringify(dailyShop({getFullYear:()=>2026,getMonth:()=>8,getDate:()=>10}))'));assert.notEqual(today,tomorrow);
