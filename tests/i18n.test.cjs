...
test('authentication and system messages have English catalog entries and use presentation keys',()=>{
 ...
 assert.match(bootstrap,/t\('auth\.loadingCompanion'\)/);assert.match(bootstrap,/t\('auth\.sessionCheckFailed'\)/);assert.match(cloud,/systemText\('system\.unsynced'\)/);assert.match(auth,/authText\('auth\.invalidCredentials'\)/);
});
