...
(async()=>{const {run,els,flush}=await setup();run('state.incubationRemaining=0;hatch(()=>0);finishBirthScene();setNickname("");state.age=DAY;state.sustained["felicidad:70"]=6*HOUR;state.care.felicidad=80;state.relationship.points=10;var r=current().rules[0]');
 ...
 const text=n=>n.textContent+n.children.map(text).join(' ');assert.ok(text(els['panel-content']).includes('Ánimo mínimo: 90'));assert.ok(text(els['panel-content']).includes('Vínculo mínimo: 60'));
 const skip=els['panel-content'].querySelectorAll('button').filter(b=>b.dataset.key?.startsWith('skip-'));assert.deepEqual(skip.map(b=>b.textContent),['+1 h','+3 h','+6 h']);assert.ok(skip.every(b=>b.children.length===0));
