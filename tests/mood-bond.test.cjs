...
(async()=>{const {run,els,flush}=await setup();run('state.incubationRemaining=0;hatch(()=>0);finishBirthScene();setNickname("");state.age=DAY;state.sustained["felicidad:70"]=6*HOUR;state.care.felicidad=80;state.relationship.points=40;var r=current().rules[0]');
 assert.equal(run('LABELS.felicidad'),'Ánimo');
 for(const [m,b,expected]of [[null,null,true],[81,null,false],[80,null,true],[null,3,false],[80,2,true],[80,3,false]]){run(`r.minMood=${m};r.minBond=${b}`);assert.equal(run('conditionsMet(r)'),expected)}
 run('r.minMood=90;r.minBond=3;var others=JSON.stringify({h:state.care.hambre,e:state.care.energia,y:state.care.higiene,v:state.vital});showPanel("oak")');
 const text=n=>n.textContent+n.children.map(text).join(' ');assert.ok(text(els['panel-content']).includes('Ánimo mínimo: 90'));assert.ok(text(els['panel-content']).includes('Vínculo ≥ 3 ♥'));
 ...
 run('var next=current().rules[0];next.minMood=50;next.minBond=1;var moodBefore=state.care.felicidad,bondBefore=state.relationship.points;forceEvolution("raichu")');assert.equal(run('state.care.felicidad'),run('moodBefore'));assert.ok(run('state.relationship.points')>=run('bondBefore'));
