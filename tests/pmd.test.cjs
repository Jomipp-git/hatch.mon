...
const ctx=vm.createContext({});for(const f of ['evolutionTable.js','hatchmonData_v2.js','pokemonDataAdapter.js','assets/pmd/manifest.js','pmdRenderer.js'])vm.runInContext(fs.readFileSync(f,'utf8'),ctx);const run=s=>vm.runInContext(s,ctx);
...
assert.equal(Object.keys(run('PMD_ASSETS')).length,44);assert.ok(sprites>44);assert.ok(portraits>44);
...
(async()=>{
 ...
 run('var canonical=PokemonData.canonicalId(state.pokemonId);var original=PMD_ASSETS[canonical];PMD_ASSETS[canonical]={sprites:{Idle:original.sprites.Idle},portraits:{}};petReaction("eat")');await flush();assert.ok(els.sprite.dataset.asset.endsWith('Idle-Anim.png'));assert.equal(els.portrait.hidden,true);
 ...
 const missing=await setup({missing:true});missing.run('state.incubationRemaining=0;hatch(()=>0);finishBirthScene();setNickname("");render();petReaction("happy")');await missing.flush();assert.equal(missing.els.sprite.dataset.visual,'placeholder');assert.equal(missing.els.portrait.hidden,true);
 console.log(`PASS PMD: ${sprites} sprite states, ${portraits} portraits, 44 canonical mappings, local routes, fallback Idle/missing files/retro and optional portraits.`);
