...
(async()=>{
 ...
 const html=fs.readFileSync('index.html','utf8');assert.ok(!html.includes('mask:var(--icon)'));for(const key of ['bag','train','oak']){assert.ok(html.includes(`data-icon="${key}"`));assert.ok(html.includes(`.pixel-icon[data-icon="${key}"]{--pixels:`));}assert.ok(html.includes('box-shadow:var(--pixels)'));assert.equal(run('typeof activateEntity'),'undefined');
