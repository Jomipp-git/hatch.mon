...
async function setup({encapsulated=false,development=true,missing=false,reduced=false,initialSave=null,initialDisplayMode=null,initialLanguage=null,initialShells=null,cloud=false,storageMap=null,runtimeSource=script,vitalSource=null,startTime=1000000}={}){
 ...
 run(fs.readFileSync('i18n.js','utf8'));run(fs.readFileSync('evolutionTable.js','utf8'));run(fs.readFileSync('hatchmonData_v2.js','utf8'));run(fs.readFileSync('pokemonDataAdapter.js','utf8'));run(vitalSource||fs.readFileSync('vitalSimulation.js','utf8'));for(const f of ['relationship.js','attentionEngine.js','pokedex.js','shiny.js','assets/skins/themes.js','shellSkins.js','styleTracing.js','trainingActivities.js','assets/pmd/manifest.js','assets/pmd/listMetrics.js','pmdRenderer.js'])run(fs.readFileSync(f,'utf8'));run(fs.readFileSync('pokemonRenderer.js','utf8'));run(fs.readFileSync('socialEngine.js','utf8'));run(fs.readFileSync('vendor/qrcode.js','utf8'));run(encapsulated?'(()=>{'+runtimeSource+'})();':runtimeSource);await flush();
