...
for(const f of ['evolutionTable.js','hatchmonData_v2.js','pokemonDataAdapter.js','vitalSimulation.js','relationship.js','pokedex.js','trainingActivities.js','socialEngine.js','vendor/qrcode.js'])run(fs.readFileSync(f,'utf8'));
...
born();run('state.inventory.berryStrength=1;useItem("berryStrength")');assert.equal(run('state.training.strength'),1);assert.equal(run('state.vital.digestion'),.35);assert.equal(run('state.pokerus'),false);run('train("strength")');assert.equal(run('state.training.strength'),6);
